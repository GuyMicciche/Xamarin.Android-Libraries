...
{
    public class EffectsController : Java.Lang.Object
    {
        ...
        public bool Apply(float factor, int effectType)
        {
            if (mEffects == null)
            {
                return false;
        ...
        private static Method ApplyTransformation
        {
            get
            {
                try
                {
                    Method m = Java.Lang.Class.FromType(typeof(Animation)).GetDeclaredMethod("applyTransformation", Float.Type, Java.Lang.Class.FromType(typeof(Transformation)));
                    m.Accessible = true;
                ...
                {
                    // we don't care because this exception should never happen
                    Console.WriteLine("WHAT? NoSuchMethodException -> " + e.Message);
                }
                return null;
            }
        }
