...
namespace ActionsContentView.Example
{
    ...
    {
        public static string TAG = Java.Lang.Class.FromType(typeof(SandboxFragment)).SimpleName;
        ...
        private IOnSettingsChangedListener mSettingsChangedListener;

        public virtual IOnSettingsChangedListener OnSettingsChangedListener
        {
            set
            {
        ...
        private class MyOnSettingSelectedListener : ValueChooserDialogFragment.IOnSettingSelectedListener
        {
            private readonly SandboxFragment fragment;
            ...
            public MyOnSettingSelectedListener(SandboxFragment fragment, View v, int id, int valueId, int valuesArrayId)
            {
            ...
            {
                int[] values = fragment.Resources.GetIntArray(ValuesArrayId);
                    ...
                    case PREF_SPACING_TYPE:
                        {
                            ...
                            string value = fragment.Resources.GetStringArray(Resource.Array.spacing_types_short)[item];
                            viewValue.Text = value;

                            // showing additional value for spacing
                            if (values[item] == ActionsContentView.SPACING_ACTIONS_WIDTH)
                            {
                                fragment.viewRoot.FindViewById(Resource.Id.prefSpacingWidthAdditionalValue).Visibility = ViewStates.Visible;
                            }
                            else
                            {
                                fragment.viewRoot.FindViewById(Resource.Id.prefSpacingWidthAdditionalValue).Visibility = ViewStates.Gone;
                            }
                            break;
                        }
                    case PREF_FADE_TYPE:
                        {
                            TextView viewValue = v.FindViewById<TextView>(ValueId);
                            string value = fragment.Resources.GetStringArray(Resource.Array.fade_types)[item];
                            viewValue.Text = value;
                            break;
                        }
                    case PREF_SWIPING_TYPE:
                        {
                            TextView viewValue = v.FindViewById<TextView>(ValueId);
                            string value = fragment.Resources.GetStringArray(Resource.Array.swiping_types)[item];
                            viewValue.Text = value;
                            break;
                        }
                    case PREF_SWIPING_EDGE_WIDTH:
                ...
                if (fragment.mSettingsChangedListener != null)
                {
                    fragment.mSettingsChangedListener.OnSettingChanged(id, values[item]);
                }
                else if (DEBUG)
                {
                    System.Console.WriteLine("OnSettingsChangedListener is not set");
